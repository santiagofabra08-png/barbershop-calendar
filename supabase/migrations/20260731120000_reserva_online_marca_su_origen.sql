-- ============================================================================
-- ARREGLO: la reserva de la página no decía de dónde salía
-- ============================================================================
-- La migración del panel agregó `appointments.source` —'online' si lo reservó
-- el cliente, 'panel' si lo cargó el barbero— y la restricción
-- `appointment_campos_coherentes` pasó a exigirlo en todo turno de cliente.
--
-- Pero `crear_reserva`, que es por donde entran TODAS las reservas de la
-- página, quedó insertando sin ese campo. Resultado: desde esa migración,
-- ninguna reserva pública entraba. El cliente veía el error de la base en
-- pantalla y no conseguía el turno.
--
-- El arreglo es una línea. Lo que falló no fue el diseño de la columna sino no
-- haber probado una reserva real después de aplicarla.
--
-- `source` no lleva DEFAULT a propósito, ni siquiera ahora. Un default 'online'
-- taparía este caso, pero se lo pondría también a los bloqueos —que no salen de
-- ningún lado y tienen que quedar en null—, y ahí la restricción fallaría del
-- otro lado. Que cada camino declare su origen es más ruidoso de escribir y más
...
begin
  ...
  begin
    insert into public.appointments (
      ...
      -- 'online' es lo único que esta función puede ser: acá entra el que
      -- reservó desde la página. Lo que carga el barbero entra por otro lado.
